...
// Helper to convert from the old LatLng format to the new {lat, lng}
function toLatLng(point: RoutePoint | { lat: () => number; lng: () => number }): RoutePoint {
    if (typeof (point as any).lat === 'function') {
        return { lat: (point as any).lat(), lng: (point as any).lng() };
    }
